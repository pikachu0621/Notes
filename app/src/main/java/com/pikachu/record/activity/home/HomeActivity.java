...
public class HomeActivity extends ReturnImagePath implements TaskAdapter.TaskActivityItemOnclick {
    ...
    private void iniView() {
        ...
        //左边点击事件
        topView.setLeftImageOnClick(new OnClickListener(){
				@Override
        ...
        //菜单点击事件
        addButtonLayout.setOnItemClick(new AddButtonLayout.OnItemClick(){
				@Override
    ...
    private void iniDrawer() {
        ...
        new Thread(new Runnable(){
				...
				public void run() {
					...
					imageViewTopBg.post(new Runnable(){
							@Override
							public void run() {
								imageViewTopBg.setImageBitmap(bitmap);
							}
						});
				}
			}).start();
        ...
        //设置item的点击事件
        drawerRecyclerAdapter.setItemOnClick(new DrawerRecyclerAdapter.ItemOnClick(){

				@Override
				public void OnClick(int position, DrawerRecyclerAdapter.ListData listData) {
    ...
    private void upDataLoadUi() {
        initialSql.querySqlData(new InitialSql.SqlData(){

				@Override
				public void sqlData(List<Mood> moods, List<Task> tasks, List<Account> accounts, List<Diary> diaries) {
    ...
    public void deleteOnClick(View view, int position, final Task task) {
        ...
        PDialog.PDialog(this)
            .setMsg("是否删除此条数据")
            .setLeftStr("删除", new PDialog.DialogTopOnClick(){
                @Override
